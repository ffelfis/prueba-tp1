...
int obtener_cantidad_palabras(FILE *archivo, int len){
    int cantidad = 0;
    char palabra[len];
    while(fgets(palabra, len, archivo)) cantidad ++;
    return cantidad;
}
...
void obtener_palabras(FILE* archivo_a_ordenar, char **lista_palabras, int len){
    ...
    while(fgets(palabra, len, archivo_a_ordenar) != NULL){
...
void orgaqsort(char** izq, char** der){
    if (izq < der) {
        ...
        char **fin = der;
        while (inicio < fin){
            while (strcmp(*inicio, *izq) <= 0)inicio++; //falta hacer que compare enteros, que seria
            while (strcmp(*fin, *izq) > 0) fin--;       //atoi(*inicio) <=/> atoi(*izq), y tambien hacer qsort general
            if (inicio < fin){
                aux = *inicio;
                *inicio = *fin;
                *fin = aux;
...
int main(int argc, char *argv[]){
    ...
    if (((strcmp(argv[2], "-o") == 0) || (strcmp(argv[2], "--output") == 0)) && ((strcmp(argv[1], "-n") == 0) || (strcmp(argv[1], "--numeric") == 0))) {
    ...
    for (int i = 0; i < cantidad_palabras; i++){
        fprintf(output, lista_palabras[i]);
        fprintf(output, "\n");
